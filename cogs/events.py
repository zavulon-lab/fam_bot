...
async def log_admin_action(bot, action_name, details, user):
    await send_log(bot, LOG_ADMIN_ACTIONS_ID, f"🛠 Админ-действие: {action_name}", details, Color.orange(), user)

async def log_user_action(bot, action_name, details, user, is_negative=False):
    col = Color.red() if is_negative else Color.green()
    await send_log(bot, LOG_USER_ACTIONS_ID, f"👥 Участники: {action_name}", details, col, user)
...
async def log_event_history(bot, event_data):
    ...
    embed = Embed(title=f"🏁 Ивент завершен: {event_data['name']}", color=0x2B2D31, timestamp=datetime.now())
...
def generate_admin_embeds(data=None):
    ...
    if data["status"] == "paused": status_text = "ПАУЗА ⏸"
    elif data["status"] == "draft": status_text = "приостановлена"
    else: status_text = "доступна ✅"
...
class OtherOptionsView(View):
    ...
    async def interaction_check(self, interaction: Interaction):
        ...
        # === WHITE LIST ===
        if val == "whitelist":
            embed = Embed(title="⭐ White List Управление", color=AUX_COLOR)
        ...
        # === WL MASS ADD ===
            ...
            embed = Embed(title="📥 Массовое добавление WL", color=AUX_COLOR)
            embed.description = (
        ...
        # === ADD RESERVE ===
            ...
            embed = Embed(title="➕ Внести в резервный список", color=AUX_COLOR)
            embed.description = (
        ...
        # === EDIT ===
            ...
            embed = Embed(title="⚙️ Редактирование", color=AUX_COLOR)
            embed.description = (
        ...
        # === PAUSE ===
            ...
            async def do_pause(inter):
                ...
                await inter.response.send_message("⏸ Регистрация ПРИОСТАНОВЛЕНА.", ephemeral=True)
        ...
        # === RESUME ===
            ...
            async def do_resume(inter):
                ...
                await inter.response.send_message("▶️ Регистрация ВОЗОБНОВЛЕНА.", ephemeral=True)
        ...
        # === KICK ===
            ...
            embed = Embed(title="🚪 Кик участника", color=AUX_COLOR)
            embed.description = (
        ...
        # === VODS ===
            ...
            embed = Embed(title="🎥 Запрос откатов", color=AUX_COLOR)
            embed.description = (
                "Пингует всех участников основы с просьбой сдать VOD.\n"
            ...
            async def do_vods(inter):
                ...
                msg_content = f"🎥 **Запрос откатов!**\n\n{pings}\n\n📤 Сдавать откаты сюда: <#{VOD_SUBMIT_CHANNEL_ID}>"
                
                target = inter.guild.get_channel(data["channel_id"])
                await target.send(msg_content)
                await log_admin_action(inter.bot, "Запрос откатов", "Пинг участников для VOD", inter.user)
...
class EventUserView(View):
    ...
    async def join(self, button, interaction):
        data = get_event_by_id(self.event_id)
        if not data: return await interaction.response.send_message("❌ Ивент не найден.", ephemeral=True)
        if data["status"] == "paused": return await interaction.response.send_message("⏸ Регистрация приостановлена.", ephemeral=True)
        if data["status"] != "active": return await interaction.response.send_message("❌ Регистрация закрыта.", ephemeral=True)
...
class MainAdminView(View):
    ...
    async def check_voice(self, button, interaction):
        ...
        if missing_ids:
            ...
            await interaction.response.send_message(f"⚠️ **Отсутствуют в войсе:**\n{missing_text}", ephemeral=True)
        else:
    ...
    async def other(self, button, interaction):
        ...
        embed = Embed(title="⚙️ Меню управления", color=0x2B2D31)
        desc = (
            "**📋 White List** — управление WL ID и массовый перенос\n"
            "**➕ Внести в резерв** — ручной ввод участников\n"
